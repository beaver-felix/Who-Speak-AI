...
\textbf{Resource-Constrained Protocol.} To complete all experimental runs within an 8-hour computational budget on an NVIDIA Tesla T4 GPU, training is conducted with one deterministic utterance per speaker per epoch (3{,}666 examples/epoch for TidyVoice, 10{,}291 examples/epoch for ViMD). Batch size is fixed at 24 following memory stress-testing against the 10{,}291-class ViMD projection layer, preserving a 20\% safety margin below GPU VRAM limits.
