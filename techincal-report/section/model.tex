This section describes the two speaker embedding architectures evaluated in our study. Both produce fixed-dimensional embeddings from variable-length audio but differ in their front-end design: ECAPA-TDNN operates on handcrafted Mel-frequency features, while RawNet3 learns directly from raw waveforms.
...
As shown in Figure~\ref{fig:ecapa-full}, 80-dim MFCCs pass through an initial Conv1D ($k\!=\!5$), then three SE-Res2Blocks with increasing dilation ($d = 2, 3, 4$). The residual connection for each block is defined as the sum of \emph{all} preceding block outputs, not just the immediate input.
...
As shown in Figure~\ref{fig:rawnet3-full}, raw waveforms ($1 \times T$) first pass through pre-emphasis, instance normalization, and a \textbf{parameterized analytic filterbank}---a learnable Conv1D ($k\!=\!251$, stride $S$) that replaces fixed MFCC extraction, followed by log compression and mean normalization.

Three AFMS-Res2MP-blocks with parameters $(k, d, p) = (3,2,5),\; (3,3,3),\; (3,3,1)$ process the output. Following ECAPA-TDNN, all block outputs are concatenated and fused by a Conv1D. \textbf{Channel and context statistics pooling} (replacing RawNet2's GRU) collapses the temporal axis into $3072 \times 1$. A FC layer produces a 256-dim embedding, trained with AAM-Softmax.
...
\midrule
Input               & 80-dim MFCC              & Raw waveform \\
